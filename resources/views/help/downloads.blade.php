...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    @if($mods->isNotEmpty())
        <div>
            <h2 class="text-base font-semibold text-white">{{ __('Mod y mapas del server') }}</h2>
            <p class="text-xs text-slate-400 mt-1 mb-3">{{ __('El mismo mod (zPAM) y paquete de mapas que tu juego descarga solo al conectarte por primera vez — bajalos acá si la descarga automática in-game va lenta.') }}</p>

            <div class="rounded-xl border border-slate-800 bg-panel overflow-hidden">
                <table class="w-full text-sm">
                    <thead>
                        <tr class="text-left text-[11px] uppercase tracking-wide text-slate-500 border-b border-slate-800">
                            <th class="px-4 py-2 font-medium">{{ __('Archivo') }}</th>
                            <th class="px-4 py-2 font-medium text-right">{{ __('Tamaño') }}</th>
                            <th class="px-4 py-2 font-medium text-right">{{ __('Acción') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($mods as $mod)
                            <tr class="border-b border-slate-800/60 last:border-0">
                                ...
                                <td class="px-4 py-2 text-right">
                                    <a href="{{ $mod['url'] }}" class="text-gsaccent hover:underline">{{ __('Descargar') }}</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
