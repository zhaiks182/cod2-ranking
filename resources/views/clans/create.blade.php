...
<div class="max-w-lg mx-auto space-y-6">
    ...
    <form method="POST" action="{{ route('clans.store') }}" enctype="multipart/form-data" class="space-y-4 rounded-xl border border-slate-800 bg-panel px-4 py-4">
        ...
        <div>
            ...
            <input type="date" name="founded_on" value="{{ old('founded_on', now()->toDateString()) }}" max="{{ now()->toDateString() }}" required class="w-full bg-panel2 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 [color-scheme:dark]">
