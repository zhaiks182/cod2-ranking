...
@php
    $statusLabels = [
        'starting' => [__('Iniciando…'), 'text-amber-400 border-amber-900 bg-amber-950/30'],
        'running' => [__('En línea'), 'text-emerald-400 border-emerald-900 bg-emerald-950/30'],
        'failed' => [__('No se pudo iniciar'), 'text-red-400 border-red-900 bg-red-950/30'],
        'stopped' => [__('Detenido'), 'text-slate-400 border-slate-700 bg-slate-800/30'],
        'expired' => [__('Expirado'), 'text-slate-400 border-slate-700 bg-slate-800/30'],
    ];
    [$statusLabel, $statusClasses] = $statusLabels[$server->status] ?? [__('Desconocido'), 'text-slate-400 border-slate-700'];
...
<div class="space-y-6">
    @if ($heroImage)
        <section class="relative rounded-2xl overflow-hidden border border-slate-800">
            ...
            <div class="relative px-4 sm:px-6 py-10 sm:py-14 text-center">
                ...
                <p class="mt-3 text-[10px] sm:text-xs md:text-sm text-slate-300 uppercase tracking-[0.15em]">{{ \App\Support\MapCatalog::mapLabel($server->map) }} · {{ __(':n jugadores', ['n' => $server->slots]) }}</p>
            </div>
        </section>
    @else
        <div class="flex items-start justify-between gap-3">
            <div>
                ...
                <p class="text-sm text-slate-400 mt-1">{{ \App\Support\MapCatalog::mapLabel($server->map) }} · {{ __(':n jugadores', ['n' => $server->slots]) }}</p>
            </div>
    ...
    @if ($server->status === 'running')
        <div class="rounded-xl border border-slate-800 bg-panel p-4 space-y-3">
            <div>
                <span class="text-xs font-medium uppercase tracking-wide text-slate-400">{{ __('Conectar por consola') }}</span>
                <div class="mt-1 flex items-center gap-2">
                    ...
                    <button type="button" onclick="cod2CopyConnect(this, {{ json_encode($connect) }})"
                        class="shrink-0 text-xs px-3 py-2 rounded-lg border border-slate-700 hover:border-cyan-500 hover:text-cyan-400 transition-colors">{{ __('Copiar') }}</button>
                </div>
            </div>

            <div>
                <span class="text-xs font-medium uppercase tracking-wide text-slate-400">{{ __('Administrar por RCON') }}</span>
                <p class="mt-1 text-xs text-slate-500">{{ __('Solo vos la ves — pegalo en la consola del juego (~) para administrar tu server.') }}</p>
            ...
            <div>
                <span class="text-xs font-medium uppercase tracking-wide text-slate-400">{{ __('Se apaga en') }}</span>
                ...
                <p class="text-[11px] text-slate-500 mt-0.5">{{ __('También se apaga solo si queda vacío por más de :n minutos.', ['n' => config('hosted_servers.idle_minutes')]) }}</p>
            </div>
        </div>

        <div class="rounded-xl border border-amber-900 bg-amber-950/30 px-4 py-3 text-xs text-amber-300">
            {{ __('Modo Search & Destroy — necesita que 2 jugadores confirmen "listo" antes de que arranque la primera ronda.') }}
        </div>

        <form method="POST" action="{{ route('hosted-servers.stop', [$server, $server->management_token]) }}"
            onsubmit="return confirm({{ json_encode(__('¿Detener este servidor ahora? No se puede deshacer.')) }})">
            @csrf
            <button type="submit" class="w-full px-4 py-2.5 rounded-lg border border-red-900 text-red-400 hover:bg-red-950/40 text-sm font-medium transition-colors">{{ __('Detener servidor') }}</button>
        </form>
    @elseif ($server->status === 'starting')
        <div class="rounded-xl border border-slate-800 bg-panel p-6 text-center text-sm text-slate-400">
            {{ __('Iniciando el servidor… esto puede tardar unos segundos. Actualizá la página en un momento.') }}
        </div>
    @else
        <div class="rounded-xl border border-slate-800 bg-panel p-6 text-center text-sm text-slate-400">
            {{ __('Este servidor ya no está activo.') }}
            <a href="{{ route('hosted-servers.create') }}" class="text-gsaccent hover:underline">{{ __('Crear uno nuevo') }}</a>.
        </div>
    @endif
</div>
...
<script>
    (function () {
        ...
        var expiredText = @json(__('Expirado'));
