        ...
        <section>
            ...
            <div class="rounded-xl border border-slate-800 bg-panel divide-y divide-slate-800/60">
                @foreach($backfilled as $match)
                    <a href="{{ route('matches.show', $match) }}" class="grid grid-cols-[3rem_1fr_auto] sm:grid-cols-[4rem_1fr_auto] items-center gap-3 sm:gap-5 px-3 py-3 sm:px-5 sm:py-4 hover:bg-slate-800/30">
                        @if($mapImageUrl = \App\Support\MapImage::url($match->map))
                            <img src="{{ $mapImageUrl }}" alt="" class="h-12 w-12 sm:h-16 sm:w-16 rounded-lg object-cover shrink-0">
                        @else
                            <div class="h-12 w-12 sm:h-16 sm:w-16"></div>
                        @endif
                        <div>
                            <div class="text-sm sm:text-base font-semibold text-white">{{ \App\Support\MapCatalog::mapLabel($match->map) }}</div>
                            <div class="text-xs sm:text-sm text-slate-500">{{ \App\Support\MapCatalog::gametypeLabel($match->gametype) }}</div>
                        </div>
                        <div class="flex items-center gap-1.5 sm:gap-2 text-sm sm:text-base shrink-0">
            ...
            <div class="rounded-xl border border-slate-800 bg-panel divide-y divide-slate-800/60">
                @foreach($dayMatches as $match)
                    <a href="{{ route('matches.show', $match) }}" class="grid grid-cols-[3rem_1fr_auto] sm:grid-cols-[4rem_1fr_auto] items-center gap-3 sm:gap-5 px-3 py-3 sm:px-5 sm:py-4 hover:bg-slate-800/30">
                        ...
                        <div class="flex items-center gap-1.5 sm:gap-3 text-sm sm:text-base shrink-0">
                            @if($match->final_score)
                                <span class="flex items-center gap-1.5">
                                    @if($match->winning_side)
                                        <span class="px-1.5 py-0.5 rounded text-xs font-medium {{ $match->winning_side === 'axis' ? 'bg-red-950/60 border border-red-800 text-red-300' : 'bg-blue-950/60 border border-blue-800 text-blue-300' }}">{{ ucfirst($match->winning_side) }}</span>
                                    @endif
