 ...
 * puerto candidato del rango y, si otro request ya se quedo con ese puerto justo
 * antes, MySQL/MariaDB rechaza el insert con una duplicate-key exception y se
 * reintenta con el siguiente -- atomico de verdad porque lo garantiza el motor de
 * base de datos, no un chequeo previo en PHP que puede quedar desactualizado entre
 * que lee y que escribe.
 */
class HostedServerPortAllocator
{
    /** @throws \RuntimeException si no hay puertos libres en el rango configurado */
    ...
    {
        // 1062 = codigo de error estandar de MySQL/MariaDB para duplicate entry en un
        // indice unique -- cualquier otro codigo es un error real, no una colision de
        // puerto, y debe propagarse tal cual.
